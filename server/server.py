"""
═══════════════════════════════════════════════════════
  AI Speech Translator — Simplified Server
...
print("╔══════════════════════════════════════════════╗")
print("║   AI Speech Translator — Server              ║")
print("║   Mobile App + ESP32 BLE Button               ║")
print("╚══════════════════════════════════════════════╝")
print()
...
def translate():
    """
    Receives audio from phone, runs AI pipeline, returns translated audio.

    Form data:
        audio: WAV/WebM audio file from phone microphone
        source_lang: source language code (e.g., "en") or "auto"
        target_lang: target language code (e.g., "ta")
        ...
        try:
            ...
            transcribe_opts = {
                ...
                "vad_parameters": dict(
                    ...
                    speech_pad_ms=200,
                    threshold=0.6,             # Higher = stricter speech detection
                ),
                "condition_on_previous_text": False,  # Prevents hallucination chains
                "no_speech_threshold": 0.5,    # Lower = stricter (rejects more non-speech)
                "log_prob_threshold": -0.8,    # Stricter: reject low-confidence gibberish
            }
            if source_lang != "auto":
            ...
            good_segments = []
            for seg in segments:
                # Skip segments where model thinks it's not speech
                ...
                if seg.avg_logprob < -1.0:
                    ...
                    continue
                # Skip very short garbage segments (single chars, punctuation)
                text = seg.text.strip()
                if len(text) < 2:
...
if __name__ == "__main__":
    ...
    print()
    print(f"📱 Open this URL on your Android phone:")
    print(f"   https://{local_ip}:5000")
    print()
    print("─────────────────────────────────────────")

    # Generate self-signed SSL cert for Web Bluetooth (requires HTTPS)
    ...
    if not os.path.exists(cert_file):
        ...
        try:
            # Try using Python's ssl module with cryptography
            ...
            print("⚠ Install 'cryptography' package for auto SSL:")
            ...
            print(f'  openssl req -x509 -newkey rsa:2048 -keyout "{key_file}" '
                  ...
                  f'-subj "/CN={local_ip}"')
            print()
            print("Starting WITHOUT SSL (Web Bluetooth won't work from phone)")
            print("─────────────────────────────────────────")
